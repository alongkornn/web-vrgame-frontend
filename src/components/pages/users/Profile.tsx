...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      try {
        ...
        console.log("API User Response:", response.data.data);
  ...
  console.log("Rendering completedCheckpoint:", user.completed_checkpoint);
  return (
    <div className="flex flex-col items-center text-white font-bold mt-10">
      ...
      <div className="mt-8 w-full max-w-lg">
        ...
        <div className="flex justify-around p-4 bg-gray-800 rounded-lg">
          {user.completed_checkpoint && user.completed_checkpoint.length > 0 ? (
            user.completed_checkpoint.map((checkpoint, index) => (
              <button key={index}>
